...
using namespace ns3;				//	use namespace std instead of using 'ns3::' everytime
...
int main (int argc, char *argv[])	//	main function
{
  ...
  */
  ...
  PointToPointHelper p2p_p2p[count];//	array of point to point helper with count number of elements
  for(int i=0;i<count;i++)
  {
	  p2p_p2p[i].SetDeviceAttribute ("DataRate", StringValue ("5Mbps"));	//	setting the attributes of p2p helper
...
///////
  ...
  ipv4.SetBase ("10.0.0.0", "255.255.255.0");	//	setting the base
...
// logic
				...
				for(int i=0;i<nc;i++)						//	revisiting lib
				{
					for(int l=1;l<3;l++)
					{
						...
						{
							echoClient = new UdpEchoClientHelper(iface_ndc_p2p[i].GetAddress (l-1), 9);
							//check++;
	
						}
					}
				}
							echoClient->SetAttribute ("MaxPackets", UintegerValue (1));				//	setting attributes
				...
				//counter++;			
				char animat[20][11];						//	generating name of the file created
				for(int p=0;p<20;p++)
				{
					for(int q=0;q<11;q++)
					{
